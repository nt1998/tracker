...
export default function App() {
  ...
  const switchTodayDay = (key) => {
    ...
    // Tell GymLog to reset its in-memory exercise index. Writing directly to
    // localStorage doesn't update GymLog's React state (useLocalStorage only
    // reads on mount), so we fire an event GymLog listens for.
    try { window.dispatchEvent(new CustomEvent('gym:reset-idx', { detail: { date: today } })) } catch { /* ignore */ }
    setTab('gym')
